...
存储服务项目的详细属性，与 services 表一对多关联，支持多子服务架构。

| 字段名 | 类型 | 约束 | 说明 | 索引 |
|---|---|---|---|---|---|
| id | uuid | PK | 服务详情唯一标识符 | PK |
| service_id | uuid | FK, NOT NULL | 关联到 services 表的ID | FK, INDEX |
| pricing_type | text | NOT NULL | 定价方式：fixed_price/hourly/by_project/negotiable/quote_based/free/custom | INDEX |
| price | numeric | | 服务价格（仅当 pricing_type 为固定价时有效） | |
| currency | text | | 货币类型 | |
| negotiation_details | text | | 议价说明（仅当定价方式为议价时有效） | |
| duration_type | text | NOT NULL | 时长类型：minutes/hours/days/visits/fixed/variable/scope_based | INDEX |
| duration | interval | | 服务时长（仅当时长类型为固定时长时有效） | |
| images_url | text[] | DEFAULT '{}' | 服务图片URLs | |
| videos_url | text[] | DEFAULT '{}' | 服务视频URLs | |
| tags | text[] | DEFAULT '{}' | 服务标签 | GIN |
| service_area_codes | text[] | DEFAULT '{}' | 服务覆盖区域编码（针对线上服务） | GIN |
...
| verification_documents | text[] | DEFAULT '{}' | 服务的相关认证文档URLs列表 | |
| created_at | timestamptz | NOT NULL, DEFAULT now() | 创建时间 | |
| updated_at | timestamptz | NOT NULL, DEFAULT now() | 更新时间 | |
| **category** | **text** | **DEFAULT 'main'** | **服务分类：main/main_course/appetizer/sushi等** | **INDEX** |
| **name** | **jsonb** | | **服务项目名称（多语言支持）** | **GIN** |
| **sub_category** | **text** | | **子分类** | |
| **is_available** | **boolean** | **DEFAULT true** | **是否可用** | **INDEX** |
| **sort_order** | **integer** | **DEFAULT 0** | **排序顺序** | **INDEX** |
| **current_stock** | **integer** | | **当前库存** | |
| **max_stock** | **integer** | | **最大库存** | |
| **attributes** | **jsonb** | **DEFAULT '{}'** | **业务属性（如辣度、烹饪时间等）** | **GIN** |
| **business_rules** | **jsonb** | **DEFAULT '{}'** | **业务规则（如配料、过敏原等）** | **GIN** | |

字段说明：
- **id**: 服务详情的唯一标识符，作为主键，支持一对多关系
- **service_id**: 关联到 services 表的ID，作为外键，支持一个服务对应多个详情项目
- **pricing_type**: 定义服务的定价方式，支持多种定价模式：固定价格、按小时、按项目、议价等
- **price**: 服务价格，仅当 pricing_type 为 'fixed_price' 时有效
- **currency**: 服务价格的货币类型
- **negotiation_details**: 当 pricing_type 为 'negotiable' 或 'quote_based' 时的议价说明
- **duration_type**: 定义服务的时长类型，支持分钟、小时、天、次数等多种单位
- **duration**: 服务时长，仅当 duration_type 为固定时长时有效
- **images_url**: 服务相关的图片URLs列表，支持多张图片展示
- **videos_url**: 服务相关的视频URLs列表，支持视频内容
- **tags**: 用于描述服务的关键词列表，方便搜索和筛选，使用GIN索引优化查询
- **service_area_codes**: 针对非物理位置服务，定义其可服务的地理区域（如邮政编码）
...
- **view_count**: 服务的浏览次数，默认值为0
- **favorite_count**: 服务的收藏次数，默认值为0
- **order_count**: 服务的订单数量，默认值为0
- **verification_status**: 服务的审核状态，如已认证、待审核、已拒绝，默认值为'pending'
- **verification_documents**: 服务的相关认证文档URLs列表，默认为空数组
- **created_at**: 服务详细信息记录的创建时间，默认值为当前时间
- **updated_at**: 服务详细信息记录的最后更新时间，默认值为当前时间

**新增字段说明（支持多子服务架构）：**
- **category**: 服务分类，用于区分不同类型的服务项目，如'main'（主要服务）、'main_course'（主菜）、'appetizer'（开胃菜）、'sushi'（寿司）等，默认值为'main'
- **name**: 服务项目名称，使用JSONB格式支持多语言，如{"zh": "宫保鸡丁", "en": "Kung Pao Chicken"}
- **sub_category**: 子分类，用于进一步细分服务类型
- **is_available**: 服务是否可用，控制服务的上下架状态，默认值为true
- **sort_order**: 排序顺序，用于控制服务项目的显示顺序，默认值为0
- **current_stock**: 当前库存数量，适用于餐饮、租赁等需要库存管理的服务
- **max_stock**: 最大库存数量，限制服务的最大供应量
- **attributes**: 业务属性，使用JSONB存储服务相关的业务属性，如辣度、烹饪时间、服务人数等
- **business_rules**: 业务规则，使用JSONB存储业务相关的规则信息，如配料、过敏原、营养信息等
