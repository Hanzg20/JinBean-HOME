...
class ImageBatchProcessor {
  ...
  Future<void> quickFixAllServiceImages() async {
    try {
      AppLogger.info(
          '[ImageBatchProcessor] Starting quick fix for all service images...');
      ...
      for (final service in services) {
        try {
          ...
          final String serviceName =
              title['zh'] ?? title['en'] ?? 'Unknown Service';
          ...
          if (existingDetails != null &&
            ...
            AppLogger.info(
                '[ImageBatchProcessor] Service $serviceId already has images, skipping...');
            continue;
          }

          // Generate category-specific images
          final int categoryId = service['category_level1_id'] ??
              service['category_level2_id'] ??
              0;
          final String category = await _getCategoryCode(categoryId);

          final List<String> images =
              _imageManager.generateTestImagesForService(
            serviceId: serviceId,
            category: category,
            count: 2,
          );
          ...
          processedCount++;
          AppLogger.info(
              '[ImageBatchProcessor] Processed service: $serviceName (ID: $serviceId)');
        } catch (e) {
          errorCount++;
          AppLogger.info(
              '[ImageBatchProcessor] Error processing service ${service['id']}: $e');
        }
      }

      AppLogger.info('[ImageBatchProcessor] Quick fix completed!');
      AppLogger.info(
          '[ImageBatchProcessor] Successfully processed: $processedCount services');
  ...
  Future<void> updateServiceImages({
    ...
    try {
      ...
      AppLogger.info(
          '[ImageBatchProcessor] Updated service $serviceId with ${images.length} images');
    } catch (e) {
      AppLogger.info(
          '[ImageBatchProcessor] Error updating service $serviceId: $e');
      rethrow;
    }
  }
  ...
  Future<void> _updateServiceDetails(
      String serviceId, List<String> images) async {
    try {
      ...
      if (existing != null) {
        ...
        // Insert new record
        await Supabase.instance.client.from('service_details').insert({
      ...
      AppLogger.info(
          '[ImageBatchProcessor] Error updating service details: $e');
      rethrow;
    }
  }
  ...
  Future<void> generateSampleImagesForTesting() async {
    try {
      AppLogger.info(
          '[ImageBatchProcessor] Generating sample images for testing...');
      ...
      for (final service in sampleServices) {
        ...
        AppLogger.info(
            '[ImageBatchProcessor] Generated ${images.length} images for ${service['name']}:');
      ...
      AppLogger.info(
          '[ImageBatchProcessor] Error generating sample images: $e');
      rethrow;
    }
  }
