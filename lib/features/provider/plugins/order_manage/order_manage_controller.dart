...
class OrderManageController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    AppLogger.info('[OrderManageController] Controller initialized',
        tag: 'OrderManage');
    loadOrders();
  }
  ...
  Future<void> loadOrders({bool refresh = false}) async {
    try {
      ...
      if (userId == null) {
        AppLogger.warning('[OrderManageController] No user ID available',
            tag: 'OrderManage');
        return;
      }
      ...
      var query = _supabase
          ...
          .range((currentPage.value - 1) * pageSize,
      ...
      if (searchQuery.value.isNotEmpty) {
        query = _supabase
            ...
            .eq('provider_id', userId)
            .or(
                'id.ilike.%${searchQuery.value}%,customer.full_name.ilike.%${searchQuery.value}%')
            .order('created_at', ascending: false)
            .range((currentPage.value - 1) * pageSize,
                currentPage.value * pageSize - 1);
      }
      ...
      AppLogger.info('[OrderManageController] Loaded ${orders.length} orders',
          tag: 'OrderManage');
    } catch (e) {
      AppLogger.error('[OrderManageController] Error loading orders: $e',
          tag: 'OrderManage');
      Get.snackbar(
        'Error',
  ...
  Future<void> updateOrderStatus(String orderId, String newStatus) async {
    try {
      ...
      AppLogger.info(
          '[OrderManageController] Order $orderId status updated to $newStatus',
          tag: 'OrderManage');
    } catch (e) {
      AppLogger.error('[OrderManageController] Error updating order status: $e',
          tag: 'OrderManage');
      Get.snackbar(
        'Error',
  ...
  Future<void> completeServiceWithClientConversion(
    ...
    try {
      ...
      AppLogger.error('[OrderManageController] Error completing service: $e',
          tag: 'OrderManage');
      Get.snackbar(
        'Error',
  ...
  /// 自动转换客户
  Future<void> _autoConvertToClient(
      String orderId, Map<String, dynamic> orderData) async {
    try {
      final customer = orderData['customer'] as Map<String, dynamic>?;
      if (customer == null) {
        AppLogger.warning('[OrderManageController] No customer data found',
            tag: 'OrderManage');
        return;
      }
      ...
      final isAlreadyClient =
          await _clientConversionService.isUserAlreadyClient(customerUserId);

      if (isAlreadyClient) {
        // 更新客户统计
        final orderAmount = (orderData['amount'] ?? 0).toDouble();
        await _clientConversionService.updateClientStats(
            customerUserId, orderId, orderAmount);
        AppLogger.info('[OrderManageController] Updated existing client stats',
            tag: 'OrderManage');
      } else {
        // 转换为新客户
        ...
        if (success) {
          ...
          AppLogger.info(
              '[OrderManageController] Customer automatically converted',
              tag: 'OrderManage');
        }
      }
    } catch (e) {
      AppLogger.error('[OrderManageController] Error in auto convert: $e',
          tag: 'OrderManage');
    }
  }

  /// 显示客户转换选择对话框
  Future<void> _showClientConversionDialog(
      String orderId, Map<String, dynamic> orderData) async {
    final customer = orderData['customer'] as Map<String, dynamic>?;
    if (customer == null) {
      AppLogger.warning('[OrderManageController] No customer data found',
    ...
    final result = await Get.dialog<bool>(
      AlertDialog(
        ...
        content:
            Text('Would you like to add "$customerName" to your client list?'),
        actions: [
          TextButton(
  ...
  Future<void> _showAutoConvertDialog() async {
    final result = await Get.dialog<bool>(
      AlertDialog(
        ...
        content: const Text(
            'Would you like to automatically convert order customers to clients in the future?'),
        actions: [
          TextButton(
            onPressed: () => Get.back(result: false),
            child: const Text('No'),
  ...
  /// 创建收入记录
  Future<void> _createIncomeRecord(
      String orderId, Map<String, dynamic> orderData) async {
    try {
      ...
      if (amount > 0) {
        ...
        AppLogger.info(
            '[OrderManageController] Income record created for order $orderId',
            tag: 'OrderManage');
      }
    } catch (e) {
      AppLogger.error(
          '[OrderManageController] Error creating income record: $e',
          tag: 'OrderManage');
    }
  }
  ...
  Future<void> _sendCompletionNotification(
    ...
    try {
      ...
      AppLogger.info(
          '[OrderManageController] Completion notification sent for order $orderId',
          tag: 'OrderManage');
    } catch (e) {
      AppLogger.error(
          '[OrderManageController] Error sending completion notification: $e',
          tag: 'OrderManage');
    }
  }
  ...
  Future<Map<String, dynamic>> getOrderStatistics() async {
    try {
      ...
      AppLogger.error('[OrderManageController] Error getting statistics: $e',
          tag: 'OrderManage');
      return {};
    }
  }
