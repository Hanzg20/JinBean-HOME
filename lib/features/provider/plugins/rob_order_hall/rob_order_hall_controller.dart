...
class RobOrderHallController extends GetxController {
  ...
  final RxList<Map<String, dynamic>> availableOrders =
  ...
  void onInit() {
    super.onInit();
    AppLogger.info('[RobOrderHallController] Controller initialized',
        tag: 'RobOrderHall');
    loadAvailableOrders();
    loadStatistics();
  }
  ...
  Future<void> loadAvailableOrders({bool refresh = false}) async {
    try {
      ...
      if (userId == null) {
        AppLogger.warning('[RobOrderHallController] No user ID available',
            tag: 'RobOrderHall');
        return;
      }
      ...
      AppLogger.info(
          '[RobOrderHallController] Loaded ${availableOrders.length} available orders',
          tag: 'RobOrderHall');
    } catch (e) {
      AppLogger.error(
          '[RobOrderHallController] Error loading available orders: $e',
          tag: 'RobOrderHall');
      Get.snackbar(
        'Error',
  ...
  Future<void> grabOrder(String orderId) async {
    try {
      ...
      AppLogger.info('[RobOrderHallController] Order accepted: $orderId',
          tag: 'RobOrderHall');
    } catch (e) {
      AppLogger.error('[RobOrderHallController] Error accepting order: $e',
          tag: 'RobOrderHall');
      Get.snackbar(
        'Error',
  ...
  Future<void> loadStatistics() async {
    try {
      ...
      final totalResponse =
          await _supabase.from('orders').select('id').eq('status', 'pending');
      ...
      final myProfileIndex =
          providerResponse.indexWhere((profile) => profile['id'] == userId);
      ...
      AppLogger.info('[RobOrderHallController] Statistics loaded',
          tag: 'RobOrderHall');
    } catch (e) {
      AppLogger.error('[RobOrderHallController] Error loading statistics: $e',
          tag: 'RobOrderHall');
    }
  }
