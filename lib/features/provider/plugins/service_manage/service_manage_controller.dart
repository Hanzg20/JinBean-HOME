...
class ServiceManageController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    AppLogger.info('[ServiceManageController] Controller initialized',
        tag: 'ServiceManage');
    loadServices();
  }
  ...
  Future<void> loadServices({bool refresh = false}) async {
    try {
      ...
      if (userId == null) {
        AppLogger.warning('[ServiceManageController] No user ID available',
            tag: 'ServiceManage');
        return;
      }
      ...
      if (searchQuery.value.isNotEmpty) {
        query = _supabase
            ...
            .eq('provider_id', userId)
            .or(
                'title->>zh.ilike.%${searchQuery.value}%,title->>en.ilike.%${searchQuery.value}%,description->>zh.ilike.%${searchQuery.value}%,description->>en.ilike.%${searchQuery.value}%')
            .order('created_at', ascending: false)
            .range((currentPage.value - 1) * pageSize,
                currentPage.value * pageSize - 1);
      }
      ...
      AppLogger.info(
          '[ServiceManageController] Loaded ${services.length} services',
          tag: 'ServiceManage');
    } catch (e) {
      AppLogger.error('[ServiceManageController] Error loading services: $e',
          tag: 'ServiceManage');
      Get.snackbar(
        'Error',
  ...
  Future<void> createService(Map<String, dynamic> serviceData) async {
    try {
      ...
      final serviceResponse = await _supabase
          ...
          .insert({
            'provider_id': userId,
            'title': {'zh': serviceData['name'], 'en': serviceData['name']},
            'description': {
            ...
            'service_delivery_method':
                serviceData['service_delivery_method'] ?? 'on_site',
      ...
      if (serviceData['details'] != null) {
        await _supabase.from('service_details').insert({
          ...
          'pricing_type':
              serviceData['details']['pricing_type'] ?? 'fixed_price',
      ...
      AppLogger.info(
          '[ServiceManageController] Service created: ${serviceResponse['id']}',
          tag: 'ServiceManage');
    } catch (e) {
      AppLogger.error('[ServiceManageController] Error creating service: $e',
          tag: 'ServiceManage');
      Get.snackbar(
        'Error',
  ...
  Future<void> updateService(
    ...
    try {
      ...
      if (serviceData['details'] != null) {
        await _supabase.from('service_details').upsert({
      ...
      AppLogger.info('[ServiceManageController] Service updated: $serviceId',
          tag: 'ServiceManage');
    } catch (e) {
      AppLogger.error('[ServiceManageController] Error updating service: $e',
          tag: 'ServiceManage');
      Get.snackbar(
        'Error',
  ...
  Future<void> deleteService(String serviceId) async {
    try {
      ...
      AppLogger.info('[ServiceManageController] Service deleted: $serviceId',
          tag: 'ServiceManage');
    } catch (e) {
      AppLogger.error('[ServiceManageController] Error deleting service: $e',
          tag: 'ServiceManage');
      Get.snackbar(
        'Error',
  ...
  Future<void> toggleServiceAvailability(
      String serviceId, bool isAvailable) async {
    try {
      ...
      AppLogger.error(
          '[ServiceManageController] Error toggling availability: $e',
          tag: 'ServiceManage');
      Get.snackbar(
        'Error',
  ...
  Future<List<Map<String, dynamic>>> getServiceCategories() async {
    try {
      ...
      AppLogger.error('[ServiceManageController] Error getting categories: $e',
          tag: 'ServiceManage');
      return [];
    }
  }
  ...
  Future<Map<String, dynamic>> getServiceStatistics() async {
    try {
      ...
      AppLogger.info('[ServiceManageController] Statistics calculated: $stats',
          tag: 'ServiceManage');

      return stats;
    } catch (e) {
      AppLogger.error('[ServiceManageController] Error getting statistics: $e',
          tag: 'ServiceManage');
      return {
        'total': 0,
        'active': 0,
