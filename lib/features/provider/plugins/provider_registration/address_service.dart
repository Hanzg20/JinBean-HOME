...
class AddressService {
  ...
  Future<String?> getOrCreateAddress(String addressInput) async {
    ...
    try {
      ...
      if (postalCode.isEmpty || streetName.isEmpty) {
        AppLogger.info(
            '[AddressService] Invalid address format: $addressInput');
        return null;
      }
      ...
      if (existingAddress != null) {
        AppLogger.info(
            '[AddressService] Found existing address: ${existingAddress['id']}');
  ...
  String _extractStreetName(String address) {
    // 移除邮编
    String cleanAddress =
        address.replaceAll(RegExp(r'[A-Z]\d[A-Z]\s?\d[A-Z]\d'), '').trim();

    // 移除城市和省份
    cleanAddress =
        cleanAddress.replaceAll(RegExp(r',\s*[^,]+,\s*[A-Z]{2}'), '').trim();

    // 提取街道部分（门牌号后的部分）
    final streetMatch = RegExp(
            r'^\d+\s+(.+?)(?:\s+(?:St|Ave|Rd|Blvd|Cres|Dr|Way|Pl|Ct|Ln|Ter|Hwy|Pkwy|Circle|Square|Court|Garden|Place|Road|Street|Avenue|Boulevard|Crescent|Drive|Way|Place|Court|Lane|Terrace|Highway|Parkway))?$',
            caseSensitive: false)
    ...
    if (streetMatch != null) {
      ...
      streetName = streetName
          .replaceAll(
              RegExp(
                  r'\s+(?:St|Ave|Rd|Blvd|Cres|Dr|Way|Pl|Ct|Ln|Ter|Hwy|Pkwy|Circle|Square|Court|Garden|Place|Road|Street|Avenue|Boulevard|Crescent|Drive|Way|Place|Court|Lane|Terrace|Highway|Parkway)$',
                  caseSensitive: false),
              '')
          .trim();
      return streetName;
  ...
  String _extractStreetType(String address) {
    final typeMatch = RegExp(
            r'\s+(St|Ave|Rd|Blvd|Cres|Dr|Way|Pl|Ct|Ln|Ter|Hwy|Pkwy|Circle|Square|Court|Garden|Place|Road|Street|Avenue|Boulevard|Crescent|Drive|Way|Place|Court|Lane|Terrace|Highway|Parkway)(?:\s|,|$)',
            caseSensitive: false)
        .firstMatch(address);
  ...
  String _extractProvince(String address) {
    final provinceRegex =
        RegExp(r'\b(ON|QC|BC|AB|MB|NB|NL|NS|NT|NU|PE|SK|YT)\b');
  ...
  bool isValidAddress(String address) {
    final hasPostalCode =
        RegExp(r'[A-Z]\d[A-Z]\s?\d[A-Z]\d').hasMatch(address.toUpperCase());
    final hasStreetNumber = RegExp(r'^\d+').hasMatch(address);
    final hasCity = address.contains(',');
    final hasProvince = RegExp(r'\b(ON|QC|BC|AB|MB|NB|NL|NS|NT|NU|PE|SK|YT)\b')
        .hasMatch(address.toUpperCase());
