...
class ClientController extends GetxController {
  ...
  Future<void> loadClients({bool refresh = false}) async {
    try {
      ...
      if (userId == null) {
        AppLogger.warning('[ClientController] No user ID available',
            tag: 'Client');
        return;
      }
      ...
      if (searchQuery.value.isNotEmpty) {
        query = _supabase
            ...
            .eq('provider_id', userId)
            .or(
                'client.full_name.ilike.%${searchQuery.value}%,client.email.ilike.%${searchQuery.value}%')
            .order('last_contact_date', ascending: false)
            .range((currentPage.value - 1) * pageSize,
                currentPage.value * pageSize - 1);
      }
      ...
      AppLogger.info('[ClientController] Loaded ${clients.length} clients',
          tag: 'Client');
    } catch (e) {
      AppLogger.error('[ClientController] Error loading clients: $e',
          tag: 'Client');
      Get.snackbar(
        'Error',
  ...
  Future<void> addClient(
      String clientId, String relationshipType, String notes) async {
    try {
      isLoading.value = true;
      ...
      AppLogger.info('[ClientController] Client $clientId added',
          tag: 'Client');
    } catch (e) {
      AppLogger.error('[ClientController] Error adding client: $e',
          tag: 'Client');
      Get.snackbar(
        'Error',
  ...
  Future<void> updateClient(
      String relationshipId, String relationshipType, String notes) async {
    try {
      isLoading.value = true;

      await _supabase.from('client_relationships').update({
        'relationship_type': relationshipType,
        'notes': notes,
        'updated_at': DateTime.now().toIso8601String(),
      ...
      AppLogger.info(
          '[ClientController] Client relationship $relationshipId updated',
          tag: 'Client');
    } catch (e) {
      AppLogger.error('[ClientController] Error updating client: $e',
          tag: 'Client');
      Get.snackbar(
        'Error',
  ...
  Future<void> addCommunication(
      String clientId, String communicationType, String content) async {
    try {
      isLoading.value = true;
      ...
      await _supabase.from('client_communications').insert({
        'provider_id': userId,
        'client_id': clientId,
      ...
      AppLogger.info(
          '[ClientController] Communication added for client $clientId',
          tag: 'Client');
    } catch (e) {
      AppLogger.error('[ClientController] Error adding communication: $e',
          tag: 'Client');
      Get.snackbar(
        'Error',
  ...
  Future<List<Map<String, dynamic>>> getClientCommunications(
    ...
    try {
      ...
      AppLogger.error('[ClientController] Error getting communications: $e',
          tag: 'Client');
      return [];
    }
  }
  ...
  Future<Map<String, dynamic>> getClientStatistics() async {
    try {
      ...
      AppLogger.error('[ClientController] Error getting statistics: $e',
          tag: 'Client');
      return {};
    }
  }
