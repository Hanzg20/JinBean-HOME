...
abstract class BasePainter extends CustomPainter {
  ...
  Path strokeAlign(String type, Path stroke, Path fill, double strokeWeight) {
    try {
      if (type == 'INSIDE') {
        Path inter =
            Path.combine(PathOperation.reverseDifference, stroke, fill);
  ...
  Path resizePathIfNeeded(Path path) {
    if (properties.containsKey('width') || properties.containsKey('height')) {
      ...
      double scalex = properties.containsKey('width')
          ? (properties['width'].toDouble()) / pathBounds.width
          : 1.0;
      double scaley = properties.containsKey('height')
          ? (properties['height'].toDouble()) / pathBounds.height
          : 1.0;
